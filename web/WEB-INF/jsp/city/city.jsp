...
<body>
<div id="toolbar">
    <%--<input class="easyui-textbox" data-options="prompt:'请输入城市名称'" style="width:25%;height:28px;" id="search_name">
...
<div class="main_table">
    <table id="dg" title="城市列表" class="easyui-datagrid"
           data-options="
...
<script>
    ...
    function newDlg() {
        ...
        $('#fm').form('clear');
        url = '<%=request.getContextPath()%>/city/add';
    }
    ...
    function removeDlg() {
        var row = $('#dg').datagrid('getSelected');
        if (row){
            $.messager.confirm('确定','您确定删除该城市吗?',function(r){
                if (r){
                    var url = "<%=request.getContextPath()%>/city/delete?id=" + row.id;
