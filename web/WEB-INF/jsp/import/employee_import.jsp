...
<table id="dg" title="导入历史" class="easyui-datagrid" style="width:100%;height:500px;"
       data-options="
                url: '<%=request.getContextPath()%>/import/get?type=4',
                method: 'get',
                idField: 'id',
                showFooter: true,
                fitColumns: true,
...
<script>
    ...
    function upload() {
        $('#uploadfrm').form('submit',{
            url: '<%=request.getContextPath()%>/import/excel?type=4',
            onSubmit: function(){
        ...
        $.messager.confirm('确定','是否开始导入?',
                function(r) {
                    ...
                    if (ifCover == 1) {
                        ...
                        url = '<%=request.getContextPath()%>/import/employee/do?ifCover=' + ifCover;
                        $.ajax({
                            type: 'post',
                            url: url,
                            data: 'data=' + json,
    ...
    function downloadModel() {
        window.location.href = "http://mr.sgcs.com.cn/shidiao/employee.xlsx";
    }
